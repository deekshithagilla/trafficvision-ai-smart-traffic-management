...
from app.config import (
    ...
    ACCIDENT_ALERT_EMAIL_COOLDOWN_MINUTES,
)
from app.utils.email_utils import send_accident_risk_email
...
# --------------------------------------------------------------------------
# Accident-risk email notification
#
# Deterministic and independent of the LLM/rule-based recommendation
# engine: the only inputs to the send decision are accident_risk_score
# (computed above from real prediction/weather/route data) and the
# configured threshold. Nothing here decides categories/severity/score -
# it only decides whether to notify given a score that was already
# computed.
# --------------------------------------------------------------------------

def _recent_accident_email_already_sent(
    db: Session, user_id: Optional[int], source: str, destination: str
) -> bool:
    """True if an accident-risk email was actually delivered (not just an
    alert created) for this user+route within the cooldown window. Using
    email_sent=True (rather than "an alert exists") means a prior SMTP
    failure never blocks a legitimate retry on the next prediction."""
...
def _weather_summary(data) -> str:
    description = (data.weather_description or "").strip()
    if description:
        return description.title()
    return data.weather_main or "Unknown"


def _maybe_send_accident_risk_email(
    db: Session,
    alert: TrafficAlert,
    ...
    try:
        if _recent_accident_email_already_sent(
            db, user_id, alert.source, alert.destination
        ):
            return
        ...
        sent = send_accident_risk_email(
            ...
            accident_risk_score=alert.accident_risk_score,
            congestion=alert.congestion,
            predicted_traffic=predicted_value,
            weather_summary=_weather_summary(data),
            expected_delay=alert.expected_delay,
        ...
        # surface as a 500 on POST /prediction/predict. The alert row
        # itself was already committed by the caller before this
        # function runs, so email_sent simply stays False and the next
        # qualifying prediction (even within the cooldown, since no
        # successful send was recorded) will retry.
        logger.exception(
            "Failed to send accident-risk email for alert id=%s "
            "(user_id=%s, route=%s -> %s)",
            alert.id, user_id, alert.source, alert.destination,
        )
        db.rollback()
