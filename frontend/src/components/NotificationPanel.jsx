...
import { getNotificationVisual, isAccidentRiskAlert } from "./NotificationCard";
...
function NotificationPanel() {
    ...
    // Group the dropdown into a dedicated "Accident Risk" section, kept
    // separate from ordinary congestion/weather/road-work alerts. Uses
    // the same classification NotificationCard already applies for the
    // 🚨/🚧 visual treatment, so this grouping never drifts out of sync
    // with what each item actually looks like.
    const accidentAlerts = alerts.filter(isAccidentRiskAlert);
    const otherAlerts = alerts.filter((alert) => !isAccidentRiskAlert(alert));
                    ...
                    {!loading && !error && alerts.length > 0 && (
                        <>
                            {accidentAlerts.length > 0 && (
                                <>
                                    <div style={sectionHeaderStyle("#991b1b", "#fef2f2")}>
                                        🚨 Accident Risk ({accidentAlerts.length})
                                    </div>
                                    {accidentAlerts.map((alert) => renderAlertItem(alert))}
                                </>
                            )}

                            {otherAlerts.length > 0 && (
                                <>
                                    <div style={sectionHeaderStyle("#334155", "#f8fafc")}>
                                        Other Alerts ({otherAlerts.length})
                                    </div>
                                    {otherAlerts.map((alert) => renderAlertItem(alert))}
                                </>
                            )}
                        </>
                    )}
...
function sectionHeaderStyle(color, bg) {
    return {
        padding: "8px 20px",
        fontSize: "11px",
        fontWeight: 700,
